...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
     ...
     */
    #[ORM\OneToMany(targetEntity: CollaborationRequest::class, mappedBy: 'collaborator', orphanRemoval: true)]
    ...
    public function addCollaborationRequest(CollaborationRequest $collaborationRequest): static
    {
        if (!$this->collaborationRequests->contains($collaborationRequest)) {
            ...
            $collaborationRequest->setCollaborator($this);
        }
    ...
    public function removeCollaborationRequest(CollaborationRequest $collaborationRequest): static
    {
        if ($this->collaborationRequests->removeElement($collaborationRequest)) {
            // set the owning side to null (unless already changed)
            if ($collaborationRequest->getCollaborator() === $this) {
                $collaborationRequest->setCollaborator(null);
            }
        }
    ...
    public function getPendingCollaborationRequests(): Collection
    {
        return $this->collaborationRequests->filter(
            fn(CollaborationRequest $request) => $request->getStatus() === \App\Enum\CollaborationStatus::PENDING
        );
    }
    ...
    {
        return in_array('ROLE_AUTHOR', $this->getRoles()) || $this->isAdmin();
    }
